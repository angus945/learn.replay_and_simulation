...
> **獨立核心，未接入 Arena。** Envelope 維持 v1。舊 game adapter 已隨 Arena 重建移除；核心與核心測試保留。不提供 transport、外部 client 或連線管理，也不是 Arena 兩個指定 framework 的必修接線。

供 External Debug Overlay、Fuzzer、AI Test 共用的 transport-neutral 協定執行核心。
純 C#、noEngineReferences；不依賴 ArenaObservation、Unity、HTTP 或特定 client。
...
Envelope 版本與 game payload 版本分開：本核心不認識遊戲型別。未來若接 Arena，應另建外圈 adapter 與明確 payload 契約；不能把 envelope v1 當作 game payload 的相容承諾。
...
保留核心與 JSON round trip／主執行緒／去重測試來源；沒有 Arena 專案 adapter，也沒有外部協定服務。
...
以下是舊 game 時期留下的紀錄，可在歷史基準 `22f6966` 查閱；不是 Arena 重建的測試結果。當次結果見 [封存的實作進度](../../docs/archive/pre-arena/implementation-progress.md)。
