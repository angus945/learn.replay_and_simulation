...
namespace Arena.Integration
{
    ...
    public sealed class ArenaMessageDescription
    {
        public ArenaMessageDescription(string type, long sequence = 0, ulong actor = 0, ulong target = 0, string detail = "")
        {
    ...
    [DataContract]
    public sealed class ArenaRecordedTraceEntry
    {
        public ArenaRecordedTraceEntry(string session, ulong tick, long sequence, string stage, string type, string code, int wave = -1, ulong actor = 0, ulong target = 0)
        {
            Session = session;
            Tick = tick;
            Sequence = sequence;
    ...
    public interface IArenaDiagnosticReader
    {
        ...
        TraceRead<ObservedFact> ReadTrace(TraceCursor cursor, int maxItems);
    }
